...
import static se.skltp.aggregatingservices.riv.clinicalprocess.healthcond.actoutcome.getlaboratoryorderoutcome.v3.Producer.TOLVAN_TOLVANSSON
...
@WebService
public class Producer1 implements GetLaboratoryOrderOutcomeResponderInterface {
	...
	public GetLaboratoryOrderOutcomeResponseType getLaboratoryOrderOutcome(
		...
		response.result.message = "Ett meddelande från HSAPRODUCER1 till användaren"
		
		def createResult = {
			response.laboratoryOrderOutcome.add(builder.createResponse(parameters, "HSAPRODUCER1"))
		}
		...
		} else if (TOLVAN_TOLVANSSON == subjectOfCareId) {
			2.times(createResult)
		}
